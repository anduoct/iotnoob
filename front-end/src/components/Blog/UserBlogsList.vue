  ...
  <div>
    ...
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> Blogs of {{ user.name || user.username }} <small v-if="blogs">(共 {{ blogs._meta.total_items }} 篇, {{ blogs._meta.total_pages }} 页)</small>
        </h3>
...
export default {
  name: 'UserBlogsList',  // this is the name of the component
  components: {
    Blog,
    Pagination
  },
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    this.getUserBlogs(to.params.id)
    // 初始化 bootstrap-markdown 插件
    $(document).ready(function() {
      $("#editForm_content").markdown({
        autofocus:false,

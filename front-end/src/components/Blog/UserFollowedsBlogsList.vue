  ...
  <div>
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> Blogs of {{ user.name || user.username }} 's followeds <small v-if="blogs">(共 {{ blogs._meta.total_items }} 篇, {{ blogs._meta.total_pages }} 页)</small>
        </h3>
...
export default {
  name: 'UserFollowedsBlogsList',  // this is the name of the component
  components: {
    Blog,
    Pagination
  },
  ...
  methods: {
    ...
    getUserFollowedsBlogs (id) {
      ...
      const path = `/api/users/${id}/followeds-blogs/?page=${page}&per_page=${per_page}`
      this.$axios.get(path)
        .then((response) => {
          // handle success
    ...
    onSubmitUpdate () {
      ...
      if (!this.editForm.content) {
        ...
        $('#editForm .md-editor').after('<small class="form-control-feedback">' + this.editForm.contentError + '</small>')
      } else {
  ...
  created () {
    ...
    this.getUserFollowedsBlogs(user_id)
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    this.getUserFollowedsBlogs(to.params.id)
    // 初始化 bootstrap-markdown 插件
    $(document).ready(function() {
      $("#editform_content").markdown({
        autofocus:false,

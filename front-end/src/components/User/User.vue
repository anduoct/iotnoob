  ...
  <section>
        ...
        <div class="row">
          ...
          <div class="col-sm-9">
            ...
            <ul class="nav nav-tabs g-mb-20">
              ...
              <li class="nav-item">
                <router-link v-bind:to="{ name: 'UserBlogsList' }" v-bind:active-class="'active'" class="nav-link">Blogs <span class="u-label g-font-size-11 g-bg-purple g-rounded-20 g-px-10">{{ user.blogs_count }}</span></router-link>
              </li>
              <li class="nav-item">
                <router-link v-bind:to="{ name: 'UserFollowedsBlogsList' }" v-bind:active-class="'active'" class="nav-link">FollowingBlogs <span class="u-label g-font-size-11 g-bg-blue g-rounded-20 g-px-10">{{ user.followed_blogs_count }}</span></router-link>
              </li>
            </ul>
    ...
    <div class="container">
      ...
      <form v-if="sharedState.is_authenticated && $route.params.id == sharedState.user_id" @submit.prevent="onSubmitAdd" class="g-mb-40">
        ...
        <div class="form-group">
          <textarea v-model="blogForm.content" class="form-control" id="blogform_content" rows="5" placeholder=" 内容"></textarea>
...
export default {
  ...
  computed: {
    isUserOverView: function () {
      const tabs = ['UserFollowers', 'UserFollowing', 'UserBlogsList', 'UserFollowedsBlogsList']
  ...
  methods: {
    ...
    onSubmitAdd (e) {
      ...
      if (!this.blogForm.content) {
        this.blogForm.errors++
        this.blogForm.contentError = 'Body is required.'

        ...
        <aside class="g-brd-around g-brd-gray-light-v4 rounded g-px-20 g-py-30">
          ...
          <div v-if="user" class="text-center g-pos-rel g-mb-30">
            <div class="g-width-100 g-height-100 mx-auto mb-3">
              <img
                class="img-fluid rounded-circle g-brd-around g-brd-gray-light-v4 g-pa-2"
                v-bind:src="user._links.avatar"
                v-bind:alt="user.name || user.username"
              />
            </div>
          ...
          <ul class="list-unstyled mb-0">
            <li class="g-pb-3">
              ...
              >
                ...
                Comments
                <span
                  v-if="notifications.unread_recived_comments_count"
                  class="u-label g-font-size-11 g-bg-pink g-rounded-20 g-px-8 g-ml-15"
                >{{ notifications.unread_recived_comments_count }}</span>
              </router-link>
            </li>
            <li class="g-py-3">
              <router-link
              ...
              >
                ...
                Messages
                <span
                  v-if="notifications.unread_messages_count"
                  class="u-label g-font-size-11 g-bg-pink g-rounded-20 g-px-8 g-ml-15"
                >{{ notifications.unread_messages_count }}</span>
              </router-link>
            </li>
            <li class="g-py-3">
              <router-link
              ...
              >
                ...
                Follows
                <span
                  v-if="notifications.unread_follows_count"
                  class="u-label g-font-size-11 g-bg-pink g-rounded-20 g-px-8 g-ml-15"
                >{{ notifications.unread_follows_count }}</span>
              </router-link>
            </li>
            <li class="g-py-3">
              <router-link
              ...
              >
                ...
                Likes
                <span
                  v-if="notifications.unread_likes_count"
                  class="u-label g-font-size-11 g-bg-pink g-rounded-20 g-px-8 g-ml-15"
                >{{ notifications.unread_likes_count }}</span>
              </router-link>
            </li>
            <li class="g-py-3">
              <router-link
                v-bind:to="{ name: 'FollowingBlogs' }"
                v-bind:active-class="'active g-color-primary--active g-bg-gray-light-v5--active'"
                class="d-block align-middle u-link-v5 g-color-text g-color-primary--hover g-bg-gray-light-v5--hover rounded g-pa-3"
              >
                <span class="u-icon-v1 g-color-gray-dark-v5 mr-2">
                ...
                Following Blogs
                <span
                  v-if="notifications.unread_followeds_blogs_count"
                  class="u-label g-font-size-11 g-bg-pink g-rounded-20 g-px-8 g-ml-15"
                >{{ notifications.unread_followeds_blogs_count }}</span>
              </router-link>
            </li>
          </ul>
          <!-- End 菜单列表 -->
          ...
          for (var i = 0; i < len; i++) {
            switch (response.data[i].name) {
              case "unread_recived_comments_count":
                this.notifications.unread_recived_comments_count =
                  response.data[i].payload;
                break;

              case "unread_messages_count":
                this.notifications.unread_messages_count =
                  response.data[i].payload;
                break;

              case "unread_follows_count":
                this.notifications.unread_follows_count =
                  response.data[i].payload;
                break;

              case "unread_likes_count":
                this.notifications.unread_likes_count =
                  response.data[i].payload;
                break;

              case "unread_followeds_blogs_count":
                this.notifications.unread_followeds_blogs_count =
                  response.data[i].payload;
                break;
            }

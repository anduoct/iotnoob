  ...
  <div>
    ...
    <form id="replyCommentForm" @submit.prevent="onSubmitReply" @reset.prevent="onResetReply" class="g-mb-40" style="display: none;">
      <div class="form-group">
        <textarea v-model="replyCommentForm.content" class="form-control" id="replyCommentFormContent" rows="5" placeholder=" 评论内容"></textarea>
        <small class="form-control-feedback" v-show="replyCommentForm.contentError">{{ replyCommentForm.contentError }}</small>
      </div>
        ...
        <div v-bind:id="'c' + comment.id" class="comment-item media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20"
          ...
          <div class="media-body">
            ...
            <div v-else class="g-mb-15">
              <h5 class="h5 g-color-gray-dark-v1 mb-0"><router-link v-bind:to="{ path: `/user/${comment.author.id}` }" class="comment-author g-text-underline--none--hover">{{ comment.author.name || comment.author.username }}</router-link> <span class="h6">在文章<router-link v-bind:to="{ name: 'BlogDetail', params: { id: comment.blog.id } }" class="g-text-underline--none--hover">《{{ comment.blog.title }}》</router-link>中写了一条新评论</span></h5>
              <span class="g-color-gray-dark-v4 g-font-size-12">{{ $moment(comment.timestamp).format('YYYY年MM月DD日 HH:mm:ss') }}</span>
            </div>
            ...
            <ul class="list-inline d-sm-flex my-0">
              ...
              <li class="list-inline-item g-mr-20">
                <router-link v-bind:to="{ path: `/blog/${comment.blog.id}#c${comment.id}` }" class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover" href="javascript:;">
...
export default {
  ...
  methods: {
    ...
    onClickReply (comment) {
      this.replyCommentForm.blog_id = comment.blog.id
    ...
    onSubmitReply () {
      ...
      if (!this.replyCommentForm.content) {
        this.replyCommentForm.errors++
        this.replyCommentForm.contentError = 'Content is required.'
        ...
        this.replyCommentForm.contentError = null
      ...
      const payload = {
        content: at_who + this.replyCommentForm.content,
        blog_id: this.replyCommentForm.blog_id,
  ...
  created () {
    ...
    $(document).ready(function() {
      $("#replyCommentFormContent").markdown({
        autofocus:false,
        savable:false,

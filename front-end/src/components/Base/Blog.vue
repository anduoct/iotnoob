<template>
  <div class="media g-brd-around g-brd-gray-light-v4 g-pa-20 g-mb-20">
    <router-link v-bind:to="{ path: `/user/${blog.author.id}` }" v-bind:title="blog.author.name || blog.author.username">
      <img class="d-flex g-width-50 g-height-50 g-mt-3 g-mr-20" v-bind:src="blog.author._links.avatar" v-bind:alt="blog.author.name || blog.author.username">
    </router-link>
    ...
    <div class="media-body">
      <div class="d-sm-flex justify-content-sm-between align-items-sm-center g-mb-15 g-mb-10--sm">
        <h5 class="h4 g-font-weight-300 g-mr-10 g-mb-5 g-mb-0--sm">
          <router-link v-bind:to="{ name: 'BlogDetail', params: { id: blog.id }}" class="g-text-underline--none--hover">{{ blog.title }}</router-link>
        </h5>
        <div class="text-nowrap g-font-size-12">
          <span>{{ $moment(blog.timestamp).fromNow() }}</span> / <router-link v-bind:to="{ path: '/user/${blog.author.id}' }"><span v-if="blog.author.name">{{ blog.author.name }}</span><span v-else>{{ blog.author.username }}</span></router-link>
        </div>
      </div>
      ...
      <div class="d-flex justify-content-start">
        ...
        <ul class="list-inline mb-0 ml-auto">
          ...
          <li v-if="blog.author.id == sharedState.user_id" class="list-inline-item g-mr-5">
            <button v-on:click="$emit('edit-blog')" class="btn btn-xs u-btn-outline-purple" data-toggle="modal" data-target="#updateBlogModal">编辑</button>
          </li>

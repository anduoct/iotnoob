<template>
  <div class="media g-brd-around g-brd-gray-light-v4 g-brd-left-1 g-pa-20 g-mb-20">
    <router-link
      v-bind:to="{ path: `/user/${blog.author.id}` }"
      v-bind:title="blog.author.name || blog.author.username"
    >
      <span v-if="blog.is_new" class="d-inline-block g-pos-rel">
        ...
        <img
          ...
          v-bind:src="blog.author.avatar"
          v-bind:alt="blog.author.name || blog.author.username"
        />
      </span>
      <img
        v-else
    ...
    <div class="media-body">
      <div class="g-mb-15">
        <h5 class="h5 g-color-gray-dark-v1 mb-0">
          ...
          >{{ blog.author.name || blog.author.username }}</router-link>
          <span class="h6">
            发布了文章
            <router-link
              v-bind:to="{ name: 'BlogDetail', params: { id: blog.id } }"
              class="g-text-underline--none--hover"
        ...
        >{{ $moment(blog.timestamp).format('YYYY年MM月DD日 HH:mm:ss') }}</span>
      </div>

      <!-- vue-markdown 开始解析markdown，它是子组件，通过 props 给它传值即可
      v-highlight 是自定义指令，用 highlight.js 语法高亮-->
      <vue-markdown :source="blog.summary" class="markdown-body g-mb-15" v-highlight></vue-markdown>
      ...
      <div class="d-flex justify-content-start">
        <ul class="list-inline mb-0">
          ...
          <li class="list-inline-item g-mr-20">
            <router-link
              v-bind:to="{ path: `/blog/${blog.id}#comment-list-wrap` }"

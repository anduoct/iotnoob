        ...
        <ul v-if="sharedState.is_authenticated" class="nav navbar-nav navbar-right">
          ...
          <li class="nav-item dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="navbarDropdown">
              ...
              <router-link v-bind:to="{ name: 'BlogsResource' }" class="dropdown-item"><i class="icon-share g-pos-rel g-top-1 g-mr-5"></i> Your resource</router-link>
...
export default {
  ...
  mounted () {
    ...
    $(function() {
      ...
      let unread_blogs_likes_count = 0  // 新收藏文章的通知计数
      let unread_comments_likes_count = 0  // 新的评论点赞的通知计数
      let unread_followeds_blogs_count = 0  // 用户关注的人的新文章通知计数
      
      setInterval(function() {
        if (window.localStorage.getItem('iotnoob-token')) {
          // 如果用户已登录，才开始请求 API
              ...
              for(var i = 0; i < response.data.length; i++) {
                switch (response.data[i].name) {
                  ...
                  case 'unread_blogs_likes_count':
                    unread_blogs_likes_count = response.data[i].payload
                    break
                  ...
                  case 'unread_followeds_blogs_count':
                    unread_followeds_blogs_count = response.data[i].payload
                    break
                }
                since = response.data[i].timestamp
              }

              total_notifications_count = unread_recived_comments_count + unread_messages_count + unread_follows_count + unread_blogs_likes_count + unread_comments_likes_count + unread_followeds_blogs_count

        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <form id="editCommentForm" @submit.prevent="onSubmitUpdateComment" @reset.prevent="onResetUpdateComment">
              <div class="form-group">
                <textarea v-model="editCommentForm.content" class="form-control" id="editCommentFormContent" rows="5" placeholder=" 评论内容"></textarea>
                <small class="form-control-feedback" v-show="editCommentForm.contentError">{{ editCommentForm.contentError }}</small>
              </div>
              <button type="reset" class="btn btn-secondary">Cancel</button>
              <button type="submit" class="btn btn-primary">Update</button>
            </form>
      ...
      <div class="col-lg-9">
        ...
        <article class="g-mb-30 g-pt-15 g-pb-15">
          <header class="g-mb-30">
            <h1 class="g-color-primary g-mb-15">{{ blog.title }}</h1>

            <ul class="list-inline d-sm-flex g-color-gray-dark-v4 mb-0">
              <li v-if="blog.author && blog.author.id == sharedState.user_id" class="list-inline-item">
                <button v-on:click="onEditBlog(blog)" class="btn btn-xs u-btn-outline-purple g-mr-5" data-toggle="modal" data-target="#editBlogModal">编辑</button>
              </li>
              <li v-if="blog.author && blog.author.id == sharedState.user_id" class="list-inline-item">
                <button v-on:click="onDeleteBlog(blog)" class="btn btn-xs u-btn-outline-red g-mr-5">删除</button>
              </li>
              <li class="list-inline-item">
                <a href="#comment-list-wrap" class="btn btn-xs u-btn-outline-aqua g-mr-10">评论</a>
              </li>
              <li v-if="blog.author" class="list-inline-item">
                <router-link v-bind:to="{ path: `/user/${blog.author.id}` }" class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover g-text-underline--none--hover"><span v-if="blog.author.name">{{ blog.author.name }}</span><span v-else>{{ blog.author.username }}</span></router-link>
              </li>
              <li class="list-inline-item g-mx-10">/</li>
              <li class="list-inline-item">
                <i class="icon-clock"></i> {{ $moment(blog.timestamp).format('YYYY年MM月DD日 HH:mm:ss') }}
              </li>
              <li class="list-inline-item g-mx-10">/</li>
              <li class="list-inline-item g-mr-10">
                <a class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover g-text-underline--none--hover" href="#comment-list-wrap">
                  <i class="icon-bubble"></i> {{ blog.comments_count }}
                </a>
              </li>
              <li class="list-inline-item ml-auto">
                <i class="icon-eye"></i> {{ blog.views }} 次阅读
              </li>
            </ul>
          ...
          <div id="blogContent" class="g-font-size-16 g-line-height-1_8 g-mb-30">
          ...
          <div id="like-blog" class="row">
            <div class="col-lg-3">
              <button v-on:click="onLikeOrUnlikeBlog(blog)" v-bind:class="btnOutlineColor" class="btn btn-block g-rounded-50 g-py-12 g-mb-10">
                <i class="icon-heart g-pos-rel g-top-1 g-mr-5"></i> 喜欢<span v-if="blog.likers_id && blog.likers_id.length > 0"> | {{ blog.likers_id.length }}</span>
              </button>
            </div>
            <div class="col-lg-9">
              <ul v-if="blog.likers" class="list-inline mb-0">
                <li class="list-inline-item"
                  v-for="(liker, index) in blog.likers" v-bind:key="index">
                  <router-link
        ...
        <div class="g-mb-60">
          <ul class="list-inline d-sm-flex mb-0">
            <li v-bind:class="{'u-pagination-v1__item--disabled': !blog.prev_id}" class="list-inline-item g-hidden-xs-down" data-toggle="tooltip" data-placement="top" title="" v-bind:data-original-title="blog.prev_title || ''">
              <router-link v-bind:to="{ name: 'BlogDetail', params: { id: blog.prev_id || 0 } }" class="u-pagination-v1__item u-pagination-v1-4 g-brd-gray-light-v3 g-brd-primary--hover g-rounded-50 g-pa-7-16" href="#" aria-label="Previous">
            ...
            <li v-bind:class="{'u-pagination-v1__item--disabled': !blog.next_id}" class="list-inline-item ml-auto g-hidden-xs-down" data-toggle="tooltip" data-placement="top" title="" v-bind:data-original-title="blog.next_title || ''">
              <router-link router-link v-bind:to="{ name: 'BlogDetail', params: { id: blog.next_id || 0 } }" class="u-pagination-v1__item u-pagination-v1-4 g-brd-gray-light-v3 g-brd-primary--hover g-rounded-50 g-pa-7-16" href="#" aria-label="Next">
                <span aria-hidden="true">
        ...
        <div id="comment-list-wrap" class="card border-0 g-mb-15">
          ...
          <form id="addCommentForm" v-if="sharedState.is_authenticated" @submit.prevent="onSubmitAddComment" @reset.prevent="onResetAddComment" class="g-mb-40">
            <div class="form-group">
              <textarea v-model="commentForm.content" class="form-control" id="commentFormContent" rows="5" placeholder=" 写下你的评论 ..."></textarea>
            ...
            <div v-for="(comment, index) in comments.items" v-bind:key="index">
              <div v-bind:id="'c' + comment.id" class="comment-item media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20">
                ...
                <div class="media-body">
                  <div class="g-mb-15">
                    <h5 v-if="comment.author.id == comment.blog.author_id" class="h5 g-color-gray-dark-v1 mb-0"><router-link v-bind:to="{ path: `/user/${comment.author.id}` }" class="comment-author g-text-underline--none--hover">{{ comment.author.name || comment.author.username }}</router-link> <button class="btn btn-xs u-btn-inset u-btn-outline-red g-mr-5">博文作者</button></h5>
                  ...
                  <ul class="list-inline d-sm-flex my-0">
                    ...
                    <ul class="list-inline mb-0 ml-auto">
                      ...
                      <li v-if="!comment.disabled && blog.author.id == sharedState.user_id" class="list-inline-item">
                      ...
                      <li v-if="comment.author.id == sharedState.user_id || blog.author.id == sharedState.user_id" class="list-inline-item">
              ...
              <div class="g-ml-40 comment-item media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20"
                ...
                <div class="media-body">
                  <div class="g-mb-15">
                    <h5 v-if="child.author.id == child.blog.author_id" class="h5 g-color-gray-dark-v1 mb-0"><router-link v-bind:to="{ path: `/user/${child.author.id}` }" class="comment-author g-text-underline--none--hover">{{ child.author.name || child.author.username }}</router-link> <button class="btn btn-xs u-btn-inset u-btn-outline-red g-mr-5">博文作者</button></h5>
                  ...
                  <ul class="list-inline d-sm-flex my-0">
                    ...
                    <ul class="list-inline mb-0 ml-auto">
                      ...
                      <li v-if="!child.disabled && blog.author.id == sharedState.user_id" class="list-inline-item">
                      ...
                      <li v-if="child.author.id == sharedState.user_id || blog.author.id == sharedState.user_id" class="list-inline-item">
...
// vue-router 从 Home 页路由到 Blog 页后，会重新渲染并且会移除事件，自定义的指令 v-highlight 也不生效了
...
export default {
  ...
  computed: {
    btnOutlineColor: function () {
      if (this.sharedState.is_authenticated) {
        if (this.blog.likers_id && this.blog.likers_id.indexOf(this.sharedState.user_id) != -1) {
  ...
  methods: {
    ...
    onSubmitUpdateBlog () {
      ...
      this.$axios.put(path, payload)
        .then((response) => {
          // handle success
          this.getBlog(this.editBlogForm.id)
    ...
    onLikeOrUnlikeBlog (blog) {
      // 用户需要先登录
      if (!this.sharedState.is_authenticated) {
        this.$toasted.error('您需要先登录才能收藏文章 ...', { icon: 'fingerprint' })
        this.$router.replace({
          path: '/login',
          query: { redirect: this.$route.path + '#like-blog' }
        })
      }

      let path = ''
      if (blog.likers_id.indexOf(this.sharedState.user_id) != -1) {
        // 当前登录用户已收藏过该文章，再次点击则取消收藏
        path = `/api/blogs/${blog.id}/unlike`
      } else {
        path = `/api/blogs/${blog.id}/like`
      }
      this.$axios.get(path)
        .then((response) => {
          // handle success
    ...
    getBlogComments (id) {
      ...
      const path = `/api/blogs/${id}/comments/?page=${page}&per_page=${per_page}`
      this.$axios.get(path)
        .then((response) => {
          // handle success
    ...
    onSubmitAddComment (e) {
      ...
      if (!this.commentForm.content) {
        ...
        this.commentForm.contentError = 'Content is required.'
        // 给 bootstrap-markdown 编辑器内容添加警示样式，而不是添加到 #blog_content 上
    ...
    onSubmitUpdateComment () {
      ...
      if (!this.editCommentForm.content) {
        this.editCommentForm.errors++
        this.editCommentForm.contentError = 'Content is required.'
        // boostrap4 modal依赖jQuery，不兼容 vue.js 的双向绑定。所以要手动添加警示样式和错误提示
        // 给 bootstrap-markdown 编辑器内容添加警示样式，而不是添加到 #blog_content 上
        $('#editCommentForm .md-editor').closest('.form-group').addClass('u-has-error-v1')  // Bootstrap 4
        $('#editCommentForm .md-editor').after('<small class="form-control-feedback">' + this.editCommentForm.contentError + '</small>')
      } else {
        this.editCommentForm.contentError = null
      }
  ...
  created () {
    ...
    this.getBlogComments(blog_id)
    // 初始化 bootstrap-markdown 编辑器
    $(document).ready(function() {
      $("#editBlogFormContent, #commentFormContent, #editCommentFormContent").markdown({
        autofocus:false,
        savable:false,
  ...
  beforeRouteUpdate (to, from, next) {
    ...
    this.getBlogComments(to.params.id)

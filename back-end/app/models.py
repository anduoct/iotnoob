    ...
    def to_dict(self, include_email=False):
        data = {
            ...
            '_links': {
                ...
                self.avatar(128),
                'followeds':
                url_for('api.get_followeds', id=self.id),
                'followers':
                url_for('api.get_followers', id=self.id),
                'blogs':
                url_for('api.get_user_blogs', id=self.id),
                'followeds_blogs':
...
db.event.listen(
    Blog.content, 'set',
    Blog.on_changed_content)  # content 字段有变化时，执行 on_changed_content() 方法
...
class Comment(PaginatedAPIMixin, db.Model):
    ...
    def to_dict(self):
        data = {
            ...
            '_links': {
                ...
                'blog_url':
                url_for('api.get_blog', id=self.blog_id),
                'parent_url':
                url_for('api.get_comment', id=self.parent.id)
    ...
    def from_dict(self, data):
        for field in [
                'content', 'timestamp', 'mark_read', 'disabled', 'author_id',
                'blog_id', 'parent_id'
        ]:
            if field in data:
                setattr(self, field, data[field])

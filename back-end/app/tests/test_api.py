...
class APITestCase(unittest.TestCase):
    ...
    def test_get_user_blogs(self):
        ...
        # 创建几篇博客
        now = datetime.utcnow()
        p1 = Blog(title='first blog from john', content='blog from john', author=u,
                  timestamp=now + timedelta(seconds=1))
        p2 = Blog(title='second blog from john', content='blog from john', author=u,
                  timestamp=now + timedelta(seconds=4))
        db.session.add_all([p1, p2])
        db.session.commit()

        headers = self.get_token_auth_headers('john', '123')
        response = self.client.get('/api/users/1/blogs/', headers=headers)
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
    ...
    def test_get_user_followed_blogs(self):
        ...
        p1 = Blog(title='first blog from david', content='blog from david', author=u2,
                  timestamp=now + timedelta(seconds=1))
        p2 = Blog(title='second blog from david', content='blog from david', author=u2,
                  timestamp=now + timedelta(seconds=4))
        p3 = Blog(title='first blog from susan', content='blog from susan', author=u3,
                  timestamp=now + timedelta(seconds=3))
        p4 = Blog(title='second blog from susan', content='blog from susan', author=u3,
                  timestamp=now + timedelta(seconds=2))
        db.session.add_all([p1, p2, p3, p4])
        db.session.commit()

        headers = self.get_token_auth_headers('john', '123')
        response = self.client.get('/api/users/1/followeds-blogs/', headers=headers)
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['_meta']['total_items'], 4)
        self.assertIsNotNone(json_response.get('items'))
        self.assertEqual(json_response['items'][0]['title'], 'second blog from david')  # 倒序排列
        self.assertEqual(json_response['items'][1]['title'], 'first blog from susan')
        self.assertEqual(json_response['items'][2]['title'], 'second blog from susan')
        self.assertEqual(json_response['items'][3]['title'], 'first blog from david')
...
class CommentsAPITestCase(unittest.TestCase):
    ...
    def test_create_comment(self):
        ...
        # 2. 缺少 content 时
        data = json.dumps({'blog_id': 1})
        response = self.client.post('/api/comments/', headers=headers, data=data)
        self.assertEqual(response.status_code, 400)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['message'], 'Content is required.')
        # 3. 缺少 blog_id 时
        data = json.dumps({'content': 'comment from john'})
        response = self.client.post('/api/comments/', headers=headers, data=data)
        self.assertEqual(response.status_code, 400)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['message'], 'Blog id is required.')
        # 4. blog_id 不正确，即找不到对应的博客文章时
        data = json.dumps({'blog_id': 2, 'content': 'comment from john'})
        response = self.client.post('/api/comments/', headers=headers, data=data)
        self.assertEqual(response.status_code, 404)
        # 5. 正确提供参数时
        data = json.dumps({'blog_id': 1, 'content': 'comment from john'})
        response = self.client.post('/api/comments/', headers=headers, data=data)
    ...
    def test_get_comments(self):
        ...
        c1 = Comment(content='first comment from john')
        c1.author = u1
        c1.blog = p
        c2 = Comment(content='second comment from john')
        c2.author = u1
        c2.blog = p
        ...
        self.assertEqual(json_response['items'][0]['content'], 'second comment from john')  # 倒序排列
        self.assertEqual(json_response['items'][1]['content'], 'first comment from john')
    ...
    def test_get_comment(self):
        ...
        # 判断返回的评论集合
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['content'], 'first comment from john')
    ...
    def test_update_comment(self):
        ...
        c1.author = u1
        c1.blog = p
        c2 = Comment(content='first comment from susan')
        c2.author = u3
        c2.blog = p
    ...
    def test_delete_comment(self):
        ...
        u3 = User(username='susan', email='susan@163.com')
        u3.set_password('123')
        p = Blog(title='first post from david', content='post from david', author=u2,
                 timestamp=datetime.utcnow() + timedelta(seconds=1))
